...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BUSD = IBEP20(0x04132bf45511d03a58afd4f1d36a29d229ccc574);
...
abstract contract DepreciatingFees {
    ...
    uint256 public marketingPenaltyFee = 4960;
    uint256 public charityPenaltyFee = 3306;
    uint256 public reflectionPenaltyFee = 3306;

    uint256 public penaltyDivisor = 10**9;
    
    uint256 public penaltyFreeTime = 1209600;
            ...
            if(transaction_amount[i] > estimateFee.__amount) {
                if(transaction_feeReduction[i]) {
                    if(penaltyFreeTime > (block.timestamp - transaction_Time[i])) {
                        estimateFee.__marketingFee =  estimateFee.__marketingFee.add(marketingPenaltyFee.mul(rest_Time).mul(estimateFee.__amount).div(estimateFee.amount));
                        estimateFee.__charityFee =  estimateFee.__charityFee.add(marketingPenaltyFee.mul(rest_Time).mul(estimateFee.__amount).div(estimateFee.amount));
                        estimateFee.__reflectionFee =  estimateFee.__reflectionFee.add(marketingPenaltyFee.mul(rest_Time).mul(estimateFee.__amount).div(estimateFee.amount));
                    ...
                    estimateFee.__marketingFee =  estimateFee.__marketingFee.add(marketingPenaltyFee.mul(penaltyFreeTime).mul(estimateFee.__amount).div(estimateFee.amount));
                    estimateFee.__charityFee =  estimateFee.__charityFee.add(charityPenaltyFee.mul(penaltyFreeTime).mul(estimateFee.__amount).div(estimateFee.amount));
                    estimateFee.__reflectionFee =  estimateFee.__reflectionFee.add(reflectionPenaltyFee.mul(penaltyFreeTime).mul(estimateFee.__amount).div(estimateFee.amount));
                    transaction_amount[i] = transaction_amount[i].sub(estimateFee.__amount);
                    transactiondata.amount = transaction_amount;
                    transactionList[estimateFee._useraddress] = transactiondata;
                    return (
                ...
                if(transaction_feeReduction[i]) {
                    if(penaltyFreeTime > (block.timestamp - transaction_Time[i])) {
                        estimateFee.__marketingFee = estimateFee.__marketingFee.add(marketingPenaltyFee.mul(rest_Time).mul(_amount).div(estimateFee.amount));
                        estimateFee.__charityFee = estimateFee.__charityFee.add(marketingPenaltyFee.mul(rest_Time).mul(_amount).div(estimateFee.amount));
                        estimateFee.__reflectionFee = estimateFee.__reflectionFee.add(marketingPenaltyFee.mul(rest_Time).mul(_amount).div(estimateFee.amount));
                    ...
                    estimateFee.__marketingFee =  estimateFee.__marketingFee.add(marketingPenaltyFee.mul(penaltyFreeTime).mul(_amount).div(estimateFee.amount));
                    estimateFee.__charityFee =  estimateFee.__charityFee.add(charityPenaltyFee.mul(penaltyFreeTime).mul(_amount).div(estimateFee.amount));
                    estimateFee.__reflectionFee =  estimateFee.__reflectionFee.add(reflectionPenaltyFee.mul(penaltyFreeTime).mul(_amount).div(estimateFee.amount));
                    estimateFee.__amount = estimateFee.__amount.sub(_amount);
                    transaction_amount[i] = 0;
                }
            }
...
contract APE is IBEP20, DepreciatingFees, Auth {
  using SafeMath for uint256;
  address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
  address DEAD = 0x000000000000000000000000000000000000dEaD;
  address ZERO = 0x0000000000000000000000000000000000000000; 
  address _presaler = 0x7889D01396c93cC8e4E5dC69ec730a8D3057A18C;
